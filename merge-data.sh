...
jq -s '.[0] as $gps | .[1] as $suppliers | $gps + $suppliers | group_by(.odsCode) | map(add) | map(select(._id))' input/gp-data.json input/remapped-suppliers.json > data/gp-data-merged.json
